...
public class RoleController {
	...
	public List<RoleVO> listByPage(RoleQuery query) {
		try {
			...
			List<RoleVO> resultRoles = ObjectCloneUtils.convertList(roles, RoleVO.class);
			return resultRoles;
		} catch (Exception e) {
	...
	public RoleVO getById(@PathVariable("id") Long id) {
		try {
			...
			List<RolePriorityRelationshipVO> resultRelations = ObjectCloneUtils.convertList(
	...
	public Boolean save(@RequestBody RoleVO role) {
		try {
			...
			List<RolePriorityRelationshipDTO> targetRelations = ObjectCloneUtils.convertList(
